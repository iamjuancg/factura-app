...
function drawTotalsBlock(doc, rows, grand, blockTitle) {
  ...
  // Titulo con lineas a ambos lados
  const titleWidth = doc.getStringUnitWidth(blockTitle) * 10 / doc.internal.scaleFactor + 6;
  const midX = boxX + boxW / 2;
  sd(doc, BORD);
  doc.setLineWidth(0.3);
  doc.line(boxX, y + 3, midX - titleWidth / 2, y + 3);
  doc.line(midX + titleWidth / 2, y + 3, W, y + 3);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  sc(doc, GRAY2);
  doc.text(blockTitle, midX, y + 4, { align: "center" });
  y += 10;

  // Linea superior del bloque
  sd(doc, BORD);
  doc.setLineWidth(0.3);
  doc.line(boxX, y, W, y);
  y += 7;

  // Filas sin linea debajo de cada una
  rows.forEach(({ label, pct, amount }) => {
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    sc(doc, GRAY2);
    doc.text(label, boxX + 2, y);
    if (pct) doc.text(pct, boxX + 32, y);
  ...
  sd(doc, BORD);
  doc.setLineWidth(0.3);
  doc.line(boxX, y + 4, W, y + 4);

  return y + 14;
}

function drawFooter(doc, iban) {
  const y = 248;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(17);
  sc(doc, GRAY1);
  ...
  doc.setFontSize(8);
  sc(doc, GRAY3);
  const lines = doc.splitTextToSize(payText, 80);
